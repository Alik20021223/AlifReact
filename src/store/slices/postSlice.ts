...
export const NEW_API_URL = "https://fe12d12ede906215.mokky.dev";
...
const postSlice = createSlice({
  ...
  reducers: {
    ...
    removePost(state, action: PayloadAction<number>) {
      ...
      axios.delete(`${NEW_API_URL}/${postIdToRemove}`)
        .then(() => {
    ...
    updatePost(state, action: PayloadAction<RecipesListProps>) {
      ...
      axios.put(`${NEW_API_URL}/${updatedPost.id}`, updatedPost)
        .then(() => {
    ...
    addPost(state, action: PayloadAction<RecipesListProps>) {
      ...
      axios.post(NEW_API_URL, newPost)
        .then(() => {
