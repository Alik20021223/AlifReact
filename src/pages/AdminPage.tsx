    ...
    <div className="container mx-auto py-10">
      ...
      <Modal isShow={showModal} title="Добавить рецепт" onClose={() => setShowModal(false)} onSave={handleCreateRecipe}>
        <div className="flex flex-col gap-4">
          ...
          />
          <Input
            placeholder="Укажите url картинки"
            value={recipe.createdAt.toString()}
            label="url"
            name="img"
            type="text"
      ...
      <Modal isShow={showChangeModal} title="Изменить рецепт" hideFooter onClose={() => setShowChangeModal(false)} onSave={handleCreateRecipe}>
        <div className="flex flex-col gap-4">
          <Input
            placeholder="Введите Название"
            value={recipe?.name || ''}
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="flex items-start gap-2">
                    <button onClick={() => handleOpenChangeModal(recipe.id)} className="px-2 rounded bg-blue-400 text-white text-lg cursor-pointer">
                      &#9998;
                    </button>
                    <button
